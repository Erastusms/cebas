...
export function PostDetailClient({ postId }: PostDetailClientProps) {
  ...
  // If the resolved post has a different ID, update URL to canonical post ID
  useEffect(() => {
      ...
      <main className="max-w-3xl mx-auto px-4 py-16 text-center space-y-4">
        ...
        <p className="text-sm text-muted-foreground max-w-md mx-auto">
          {error instanceof Error
            ? error.message
            : "Postingan yang Anda cari tidak tersedia, telah dihapus, atau dibatasi."}
        </p>
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-6 sm:py-8 space-y-6">
      ...
      <div className="flex items-center space-x-3">
        ...
        <h1 className="text-lg font-bold text-foreground">Postingan</h1>
      </div>
      ...
      <section className="space-y-3" aria-label="Kirim balasan ke postingan">
        <ReplyComposer
          postId={post.id}
